...
- **18:15-18:22 — RELEASE EXECUTED.** Merged model uploaded + verified on Hub: `centuriandip/llama-3.1-8b-tools-sft` rev 8a606eda, model.safetensors 16.06GB (console showed 2.84GB transferred — HF Xet chunk-dedup vs base weights, not truncation). Adapter + trainer_state archived (adapter/ folder in model repo; week4_trainer_state.json in staging dataset). Model card finalized. Release checklist completed and SIGNED via release-kit: sha256 45fbc788, approver dipak, verify OK. Pod stopped ~18:22. Commits: fc1412f (llama-tools), de1b89a (release-kit).
- **18:23 — Week 5 data gate CLOSED (human adjudication).** Dipak ruled on all 6 flagged pairs: agree with triage on all, drop pair 71 (xlam-2648, adult content) ahead of the planned public dataset release. `EXCLUDED_SOURCE_IDS` added to validate_preferences.py (shared with clean_sft.py). Regenerated: **DPO final 10,242** (was 10,243); **SFT v2 12,143** (was 12,144; 17 total dropped = 16 corrupt + 1 excluded). Both re-uploaded to HF staging. Method on record: model-assisted triage (200 pairs) + human adjudication of flags.

- **18:42 — Week 5 DPO smoke: PASSED** (new pod 891565153d18; one TRL-drift fix required first, d22ce8b: max_prompt_length removed from DPOConfig). 32 steps / 9 min / 500 pairs: loss 0.19-0.27 (avg 0.365; <0.693, not collapsed), rewards/accuracies 1.0, margins 1.33-1.67, rewards/chosen ≈ 0 (no policy degradation; contrast gained by pushing rejected down to ≈ −1.3, logps/rejected ≈ −40).
- **18:43 — Pre-registered evaluation framing for the full DPO run (before launch, so we can't grade ourselves on an easy metric):** pinned accuracies are EXPECTED and prove pipeline health only — rule-based rejecteds are mechanically easy for an SFT-ed policy. Genuine signal lives in the 1,654 wrong_arg_value pairs (the en-US/pt-BR class). Success criteria are downstream only: (a) DPO model fixes argument-grounding misses in sample review, (b) BFCL delta vs SFT-only in Week 7. Contingency if BFCL delta is weak: v2 preference set with model-generated rejecteds (harder negatives), Week 8+ only if evidence demands.

<!-- Week 5-6 entries continue below. -->
