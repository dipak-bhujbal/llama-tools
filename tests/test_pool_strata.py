...
from mining.pool_strata import MULTI, SINGLE, UNPARSEABLE, composition, stratum_of, tool_count
...
def test_xlam_format_is_counted() -> None:
    ...
    assert stratum_of(XLAM_TWO) == MULTI
    assert stratum_of(XLAM_ONE) == SINGLE
...
def test_hermes_format_is_counted() -> None:
    ...
    assert stratum_of(HERMES_THREE) == MULTI
    assert stratum_of(HERMES_ONE) == SINGLE
...
def test_a_prompt_with_no_recognisable_tool_list_is_unparseable() -> None:
    assert tool_count("You are a helpful assistant. No tools here.") is None
    assert stratum_of("You are a helpful assistant.") == UNPARSEABLE
...
def test_composition_reports_counts_with_the_input_digest(tmp_path: Path) -> None:
    ...
    assert receipt["counts"] == {MULTI: 2, SINGLE: 1, UNPARSEABLE: 1}
